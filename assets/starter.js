...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const toggleFavorite = (jobId) => {
        ...
        if (index != -1) {
            ...
            const careToRemove = document.querySelector(`[data-job-id="${jobId}"]`)
            careToRemove.remove()
    ...
    const renderJobs = (jobsToRender) => {
        ...
        for (let button of buttons) {
            button.addEventListener("click", () => {
                console.log(button.getAttribute("data-job-id"))
                toggleFavorite(Number(button.getAttribute("data-job-id")));

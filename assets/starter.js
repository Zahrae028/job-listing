...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const toggleFavorite = (jobId) => {
        ...
        // 4. Update UI
        const buttons = document.getElementsByClassName("job-card__favorite-btn");
        
        for(let button of buttons){
            button.addEventListener("click",(e)=>{

                let idnum = e.target.getAttribute("data-job-id");
                console.log(idnum);
                ...
                if (favoriteJobIds.includes(idnum)==true) {
                    let index = favoriteJobIds.indexOf(idnum)
                     favoriteJobIds.unshift(index, idnum);
                }else{

                    favoriteJobIds.push(Number(idnum));
                }
  ...
  searchInput.addEventListener("input" , (event)=>{
            let searchedInput = event.target.value.toLowerCase();

            const filtered = [];

            for (let i = 0; i < allJobs.length; i++) {
                if (allJobs[i].position.toLowerCase().includes(searchedInput.toLowerCase()) || allJobs[i].company.toLowerCase().includes(searchedInput.toLowerCase()) || allJobs[i].location.toLowerCase().includes(searchedInput.toLowerCase())) {
                    filtered.push(allJobs[i]);
                }
            }
